...
export default class SearchProvider {
  ...
  async findReferences(token, context) {
    const rg = this.toolResolver.getRg();
    if (!rg.available || !token || !context.projectRoot) return [];
  ...
  parseVimgrep(stdout, projectRoot, token, options = {}) {
    const candidates = [];
    ...
    for (const line of lines) {
      ...
      const kind = options.kind || inferKind(lineText);
    ...
    return candidates.map((candidate) => ({
      ...candidate,
      displayName: candidate.source === 'rg-reference'
        ? candidate.lineText
