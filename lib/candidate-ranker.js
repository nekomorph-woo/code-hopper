...
export default class CandidateRanker {
  rank(candidates, context, options = {}) {
    ...
    if (options.limit === false) return ranked;
  ...
  score(candidate, context) {
    ...
    if (candidate.source === 'rg-reference') score += 12;
    ...
    if (context.openFilePaths && context.openFilePaths.has(normalize(candidate.filePath))) score += 10;
    if (context.gitModifiedPaths && context.gitModifiedPaths.has(normalize(candidate.filePath))) score += 14;
