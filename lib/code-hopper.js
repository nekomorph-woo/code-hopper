...
export default {
  config: {
    ...
    sourceRoots: {
      ...
      default: [],
      items: { type: 'string' },
      description: 'Optional project-relative directories to index. Leave empty to auto-detect common source roots.'
    },
    indexDirectory: {
      type: 'string',
      default: '',
  ...
  activate(state = {}) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'code-hopper:search-symbol': () => this.searchWorkspaceSymbols(),
      'code-hopper:find-references': () => this.findReferences()
    }));
  ...
  async findCandidates(context) {
    const rankingContext = await this.enrichRankingContext(context);
    const indexedCandidates = this.indexManager.query(context.token, context);
    const fallbackCandidates = await this.searchProvider.findDefinitions(context.token, context);

    return this.candidateRanker.rank([...indexedCandidates, ...fallbackCandidates], rankingContext);
  },

  async searchWorkspaceSymbols() {
    const editor = atom.workspace.getActiveTextEditor();
    const projectRoot = editor && editor.getPath()
      ? this.projectRootForPath(editor.getPath())
      : atom.project.getPaths()[0];

    if (!projectRoot) {
      atom.notifications.addInfo('Code Hopper: no project is open.');
      return;
    }

    const context = await this.enrichRankingContext({
      token: '',
      filePath: editor && editor.getPath ? editor.getPath() : projectRoot,
      projectRoot,
      language: editor && editor.getPath ? languageFromFile(editor.getPath()) : null,
      imports: [],
      packageName: null,
      isTestFile: false
    });

    const candidates = this.candidateRanker.rank(this.indexManager.listSymbols(projectRoot), context, {
      limit: false
    });
    if (candidates.length === 0) {
      atom.notifications.addInfo('Code Hopper: no indexed symbols yet. Refresh the index first.');
      return;
    }
  ...
  async findReferences(editor = atom.workspace.getActiveTextEditor(), bufferPosition = null) {
    const context = this.buildJumpContext(editor, bufferPosition);
    if (!context || !context.token) {
      atom.notifications.addInfo('Code Hopper: no symbol under cursor.');
      return;
    ...
    const references = await this.searchProvider.findReferences(context.token, context);
    const ranked = this.candidateRanker.rank(references, await this.enrichRankingContext(context));

    if (ranked.length === 0) {
      atom.notifications.addInfo(`Code Hopper: no references for "${context.token}".`);
      return;
    }

    this.candidateListView.show(ranked, (candidate) => this.openCandidate(candidate), {
      placeholder: `Filter references for ${context.token}`
    });
  },
  ...
  async enrichRankingContext(context) {
    return {
      ...context,
      openFilePaths: this.getOpenFilePaths(),
      gitModifiedPaths: await this.getGitModifiedPaths(context.projectRoot)
    };
  },
  ...
  getOpenFilePaths() {
    ...
    for (const pane of atom.workspace.getPanes()) {
      for (const item of pane.getItems()) {
        if (item && item.getPath && item.getPath()) {
          filePaths.add(path.normalize(item.getPath()));
        }
      }
    }
  ...
  getGitModifiedPaths(projectRoot) {
    return new Promise((resolve) => {
      ...
      execFile('git', ['-C', projectRoot, 'status', '--porcelain'], { timeout: 3000 }, (error, stdout) => {
        if (error) {
          resolve(new Set());
          return;
        }
        ...
        stdout.split(/\r?\n/).forEach((line) => {
          ...
          if (rawPath) paths.add(path.normalize(path.join(projectRoot, rawPath)));
        });
